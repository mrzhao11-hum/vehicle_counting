"""训练CARPK B2：使用完整CSRNet教师蒸馏1/4-CSRNet学生。

本入口复现原SKT公开实现的四项损失：

    L = L_GT + L_output + 0.5 * L_FSP + 0.5 * L_cos

教师始终冻结，学生从与B1相同的Kaiming随机初始化开始训练。1x1特征适配
层只服务于训练期蒸馏并单独保存，最终``model_state``仅包含轻量学生主干，
因此可以直接交给``evaluate_student.py``和``analyze_model.py``。

小样本链路检查：

    python train_skt.py --config configs/carpk_student_skt_fixed.yaml \
        --overfit-samples 8 --epochs 5 \
        --output-dir outputs/overfit/b2_student_skt_fixed_8
...
from engine.skt_trainer import train_skt_one_epoch
from losses import DensityMSELoss, SKTFeatureAdapters
from models import CSRNetStudent, CSRNetTeacher, TEACHER_FEATURE_CHANNELS


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="训练CARPK B2 SKT蒸馏学生")
    parser.add_argument("--config", type=Path, required=True, help="B2 YAML配置文件")
    parser.add_argument("--output-dir", type=Path, default=None, help="覆盖实验输出目录")
    parser.add_argument("--resume", type=Path, default=None, help="从B2 checkpoint继续训练")
    parser.add_argument(
        "--teacher-checkpoint",
        type=Path,
        default=None,
...
def main() -> None:
    ...
    if args.overfit_samples > 0:
        ...
        print(f"启用B2蒸馏链路检查：训练和验证使用同一组{sample_count}张图")
    ...
    if resume_path is not None:
        ...
        if "adapter_state" not in checkpoint:
            raise KeyError("B2 checkpoint缺少adapter_state，无法恢复蒸馏训练")
        ...
        print(f"已恢复B2 checkpoint：{resume_path}，从epoch {start_epoch}继续")
    ...
    if any(
        ...
        raise ValueError("四项蒸馏损失权重均不能为负数")
    ...
    print(f"设备：{device}")
    print(f"模型：1/{channel_ratio}-CSRNet学生（B2，原始SKT蒸馏）")
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        train_metrics = train_skt_one_epoch(
            teacher=teacher,
            student=student,
            adapters=adapters,
            loader=train_loader,
        ...
        validation_metrics = evaluate_model(
            ...
            amp_enabled=amp_enabled,
            description=f"B2 SKT validation epoch={epoch:03d}",
        )
        ...
        checkpoint_payload = {
            "epoch": epoch,
            "experiment": "B2 quarter-CSRNet with original SKT distillation",
    ...
    print(f"B2 SKT训练完成，最佳验证MAE={best_mae:.3f}")
